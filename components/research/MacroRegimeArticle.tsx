...
import labor from "@/content/research/2026-05-30-macro-regime/data/labor.json";
import growth from "@/content/research/2026-05-30-macro-regime/data/growth.json";
...
import summary from "@/content/research/2026-05-30-macro-regime/data/summary.json";
    ...
    <article className="mx-auto max-w-[860px] pb-16">
      ...
      <ul className="mb-4 ml-1 space-y-1.5 font-mono text-[13px] leading-[20px] text-text">
        {[
          ...
          "But watch the soft data: Michigan sentiment sits 1.8σ below its decade mean while the hard data is broadly solid — the widest hard/soft divergence outside a recession.",
        ].map((t, i) => (
      ...
      <P>
        ...
        low at 4.3%, claims subdued near <Em>215k</Em>, industrial production growth back in positive
        territory, the Sahm rule dormant at 0.13 (a recession trigger is 0.50), and the Fed easing
        (funds 3.6%, down ~70bp y/y). The discordant note is consumer sentiment, at a{" "}
      ...
      <P>
        ...
        near <Em>215k</Em> (Fig. 2). The Sahm rule, which has called every modern recession, sits at
      ...
      <Section n="04" title="Growth: noisy survey, steadier hard data" />
      <P>
        Since ISM’s PMI is no longer freely redistributable, I proxy the factory pulse with the Philly
        Fed manufacturing diffusion index — same construction, same signal, and a useful reminder of
        how noisy survey data is. After a sharp rebound earlier in the year, the May print faded back
        to roughly flat (<Em>−0.4</Em>); single-month survey swings are mostly noise. The steadier read
        is the hard data: industrial-production growth has turned positive again at{" "}
        <Em>+1.4%</Em> YoY (Fig. 3), the goods cycle grinding out of its 2024 contraction. That, not
        the jumpy survey, anchors the growth half of the regime call.
      </P>
      ...
      <P>
        Today the model places us in <Em>{cur?.name}</Em> — growth firming in the hard data (Fig. 3) and core
      ...
      <div className="space-y-2 font-mono text-[11px] leading-[18px] text-text-dim">
        <p><span className="text-text-faint">Source.</span> FRED (no key): PAYEMS, UNRATE, ICSA, CPIAUCSL, CPILFESL, PCEPI, PCEPILFE, RSAFS, INDPRO, FEDFUNDS, NFCI, UMCSENT, SAHMCURRENT, T10Y3M, and the Philly Fed manufacturing index (ISM-PMI proxy). VIX from Yahoo Finance (^VIX); equities Yahoo SPY (monthly). FRED occasionally rate-limits its heaviest daily series; when the 10y–3m curve is unavailable at a refresh it is omitted rather than proxied.</p>
